...
def match(args):
    ...
    STEPS_PER_EPOCH = 100
    min_loss = 999999999   #最小损失函数值
    min_arg = 0     #最小损失函数值对应的风格图片
    for i in range(4):  #五个默认风格图片
        if i == 0:
            st_image = os.path.join(args.style_image, 'candy.jpg')
        elif i == 1:
            st_image = os.path.join(args.style_image, 'mosaic.jpg')
        elif i == 2:
            st_image = os.path.join(args.style_image, 'rain-princess.jpg')
        elif i == 3:
            st_image = os.path.join(args.style_image, 'udnie.jpg')
        style = utils.load_image(st_image, size=args.style_size)
                ...
                for step in range(STEPS_PER_EPOCH):
                    for batch_id, (x, _) in enumerate(train_loader):
                        ...
                        if (batch_id + 1) % args.log_interval == 0:
                            mesg = "{}\tEpoch {}_\t[{}/{}]\tcontent_ {_.6f}\tstyle_ {_.6f}\ttotal_ {_.6f}".format(
        ...
        # save model
        # transformer.eval().cpu()
        # save_model_filename = "style_"+ str(i) + ".pth"
    ...
    if args.model.endswith(".onnx"):
        ...
        with torch.no_grad():
            ...
            if min_arg == 0:
                model_path = os.path.join(args.model, 'candy.pth')
            elif min_arg == 1:
                model_path = os.path.join(args.model, 'mosaic.pth')
            elif min_arg == 2:
                model_path = os.path.join(args.model, 'rain_princess.pth')
            elif min_arg == 3:
                model_path = os.path.join(args.model, 'udnie.pth')
    ...
    save_path = os.path.join(args.output_image,str(time.ctime()).replace(' ', '_').replace(':', '.'))
