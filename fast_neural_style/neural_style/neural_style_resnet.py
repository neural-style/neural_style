    ...
    for e in range(args.epochs):
        ...
        with tqdm(total=steps_per_epoch, desc='Epoch {}/{}'.format(e + 1, args.epochs)) as pbar:
            for batch_id, (x, _) in enumerate(train_loader):
                ...
                # 将 原始内容图片 和 生成混合图片 通过 ResNet18 获得各层输出结果 方便进行 loss 计算
                features_y = resnet(y)
                features_x = resnet(x)

                # 计算 内容损失 使用 ResNet18 的 layer3 来计算 论文中指出高层输出能够更好的代表内容特征
                content_loss = args.content_weight * mse_loss(features_y.layer3, features_x.layer3)

                # 计算风格特征
                style_loss = 0.
                # 使用 ResNet18 每一个模块的输出 论文中指出 风格特征可以从每一个模块的不同卷积通道之间的相似度来提取
